...
class TurnaroundDaybreakV2(strategyBase.StrategyBase):
    ...
    def day_end(self):
        ...
        if not self.isCurrentlyLong[0] and self.goLongAtOpen[ticker]:
            self.quantity = int(self.mediator.get_account_information().cash_balance // self.todayClose[ticker]) * 3
            buy = OrderCommand(ticker=ticker, quantity=self.quantity)
            #print(f"\nBuying {self.quantity} shares of {ticker} at the open price of {self.todayClose[ticker]}\n")
            self.mediator.buy_market_order(buy)
            self.isCurrentlyLong = (True, ticker)
