...
class C_Lab(strategyBase.StrategyBase):
    ...
    def check_for_trade(self):
        ...
        #             if (self.isInMarket and not self.is_shorting):
        #                 self.mark_for_sell = False
        ...
        #                 self.buy_price = self.todayClose
        ...
        if self.todayDateTime.weekday() in self.days:
            if len(self.priceHistory) >= 2:
                yesterday = self.priceHistory[-1]
                two_days_ago = self.priceHistory[-2]
                if self.todayClose > yesterday[2] > two_days_ago[2]:
                    if (self.isInMarket and self.is_shorting):
                        self.mark_for_sell = False
                    print("Filter conditions HIT (SHORT)===========================================================")
                    if not self.isInMarket:
                        print(f"Cash Balance: {self.mediator.get_account_information().cash_balance}")
                        print(f"self.todayClose: { self.todayClose}")
                        self.quantity = math.floor(int(self.mediator.get_account_information().cash_balance // self.todayClose))
                        print(f"Quantity = {self.quantity}")
                        buy = OrderCommand(ticker=current_ticker, quantity=self.quantity)
                        self.mediator.short_open_market_order(buy)
                        self.buy_price = self.todayClose
                        print(f"Shorting {self.quantity} shares of {current_ticker} at the price of {self.todayClose}===========================")
                        self.isInMarket = True
        ...
        #                     print(self.mediator.get_account_information().cash_balance)
        #                     exit()
        #                 short = OrderCommand(ticker=current_ticker, quantity=self.quantity)
        #                 self.mediator.short_open_market_order(short)
        #                 print(f"Shorting {self.quantity} shares of {current_ticker} at the price of {self.todayClose}~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        #                 self.isInMarket = True
        #                 self.is_shorting = True
        #         else:
        ...
        if (self.mark_for_sell):
            if (self.is_shorting):
                ...
                self.mediator.short_close_market_order(sell)
                print(f"Closing short position of {self.quantity} shares of {current_ticker} at the close price of {self.todayClose}~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            # else:
            #     sell = OrderCommand(ticker=current_ticker, quantity=self.quantity)
    ...
    def day_end(self):
        ...
        if (self.isInMarket):
            ...
            # else:
            #     percentage_change = ((self.todayClose - self.buy_price) / self.buy_price) * 100
            #     #exit the trade when the close is higher than yesterday's high
            #     print(f"{abs(percentage_change) <= 1} and {self.days_in_market < 3}")
            #     if (abs(percentage_change) <= 1 and self.days_in_market < 3):
            #         self.mark_for_sell = False
            #     elif self.todayClose > self.priceHistory[-1][1]:
            #         self.mark_for_sell = True
    ...
    def on_tick_changed(self, frame: TickChangeData):
        ...
        if frame.price < self.todayLow:
            self.todayLow = frame.price
    ...
    def on_order_changed(self, frame: strategyBase.OrderChangeData):
        ...
        print(f"ORDER CHANGED {frame.order_status}")
        pass
    ...
    def get_adapt_day(self):
        stock_data = get_stock_data(current_ticker, self.datetime_utc - timedelta(days=90), self.datetime_utc)
...
def calculate_returns(stock_data):
    ...
    for i in range(len(stock_data) - 1):
        today_low = stock_data['Low'].iloc[i]
        for j in range(i + 1, len(stock_data)):
            if stock_data['Close'].iloc[j] > stock_data['Low'].iloc[j-1]:
                stock_data.loc[stock_data.index[i], 'Return'] = (stock_data['Close'].iloc[j] - today_low) / today_low
                break
