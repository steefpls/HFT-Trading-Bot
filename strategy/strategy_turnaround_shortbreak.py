...
class TurnaroundShortbreak(strategyBase.StrategyBase):
        ...
        if self.todayDateTime.weekday() == 1 or self.todayDateTime.weekday() == 2:
            ...
            if len(self.priceHistory) >= 3:
                ...
                if yesterday[2] < two_days_ago[2] and two_days_ago[2] < three_days_ago[2]:
                    if not (self.isCurrentlyLong or self.isCurrentlyShort):
                        self.shouldGoLong = True
                        self.shouldGoShort = False
                elif not (self.isCurrentlyLong or self.isCurrentlyShort):
                    self.shouldGoLong = False
                    self.shouldGoShort = True
            else:
    ...
    def day_end(self):
        ...
        #brief_query = StockBriefsQuery()
        #brief_query.tickerList = [current_ticker]
        #brief = self.mediator.query_stock_briefs(brief_query)

        #self.todayHigh = brief.get_price(current_ticker, PriceType.HIGH)
        #self.todayClose = brief.get_price(current_ticker, PriceType.CLOSE)

        if self.todayClose==0:
            return
        ...
        if not (self.isCurrentlyLong or self.isCurrentlyShort):
            if(self.shouldGoLong):
                ...
                self.isCurrentlyShort = True

        elif should_sell and self.isCurrentlyLong:
                #sell
        ...
        elif should_buyback and self.isCurrentlyShort:
                #buyback
                sell = OrderCommand(ticker=current_ticker, quantity=self.quantity)
