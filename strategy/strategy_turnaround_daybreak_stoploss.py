...
class TurnaroundStopLoss(strategyBase.StrategyBase):
    ...
    def init(self):
        ...
        self.shouldGoLong = False

        self.shouldStopLoss = False
    ...
    def day_end(self):
        ...
        if(self.shouldGoLong and not self.isCurrentlyLong):
            print("buy at end")
            ...
            print(f"Buying {self.quantity} shares of {current_ticker} at the open price of {self.todayClose}")
...
#aoyun gap
    ...
    def on_order_changed(self, frame: strategyBase.OrderChangeData):
        ...
        print(f"{frame.order_type} {frame.action} order is {frame.order_status} with order id of {frame.orderID}")

        if frame.action == OrderAction.CANCEL:
            print(f"order {frame.orderID} is cancelled for real this time")
        match(frame.order_type):
            case OrderType.STOP:
                if frame.order_status == OrderStatus.NEW:
                    self.current_stop_order_id = frame.orderID
                    print("stop loss order placed")

            case OrderType.MARKET:
                if frame.action == OrderAction.SELL and frame.order_status == OrderStatus.FILLED:
                    self.isCurrentlyLong = False
                    print(f"cash balance: {self.mediator.get_account_information().cash_balance}")
                
                elif frame.action == OrderAction.SELL and frame.order_status == OrderStatus.NEW:
                    self.current_exit_id = frame.orderID
                    #cancel = CancelOrderCommand(self.current_stop_order_id)
                    #self.mediator.cancel_order(cancel)
                    #print(f"order {self.current_stop_order_id} cancelled")
                    #self.current_stop_order_id = -1
                    
                if frame.order_status == OrderStatus.FILLED and frame.action == OrderAction.BUY:
                    #place stop loss order
                    self.fill_price = frame.avg_fill_price
                    self.shouldStopLoss = True
                    self.isCurrentlyLong = True
    ...
    def submit_stop_loss(self):
        print(f"{self.quantity} stocks bought")
        print(f"placing stop loss order for {self.quantity} stocks")
        stop_loss = OrderCommand(ticker = current_ticker, aux_price=self.fill_price*0.9, quantity = self.quantity)
        self.mediator.stop_market_sell_order(stop_loss)
        self.shouldStopLoss = False
